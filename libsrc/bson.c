...
int bson_copy( bson *out, const bson *in ) {
    ...
    out->cur = out->data + bson_size( out );
    out->finished = 1;
...
int bson_init_data( bson *b, char *data ) {
    b->data = data;
    b->dataSize = bson_size( b );
    b->cur = b->data + b->dataSize;
    return BSON_OK;
}
...
int bson_iterator_size( const bson_iterator *i ) {
    bson_iterator next = *i; bson_iterator_next(&next);
    int size = next.cur - i->cur;

    return size - 2 - strlen( bson_iterator_key( i ) );
}
...
int bson_append_bson( bson *b, const char *name, const bson *bson ) {
    return bson_append_value(b, name, BSON_OBJECT, bson->data, bson_size( bson ));
}

int bson_append_value( bson *b, const char *name, bson_type type, const char *buffer, int size) {
    if ( bson_append_estart( b, type, name, size ) == BSON_ERROR )
        return BSON_ERROR;
    bson_append( b, buffer, size );
    return BSON_OK;
}
